...
- **100% офлайн** — ни API-ключей, ни интернета после конвертации модели
- **faster-whisper (CTranslate2)** — около 60× быстрее реального времени на потребительской видеокарте с 16 ГБ; запись длиной 2 ч 52 мин расшифровывается примерно за 2,5 минуты
...
- **3 профиля** — Быстрый · Точный · Шумное аудио
...
Файлов зависимостей четыре, у каждого своя задача:

| Файл | Когда нужен |
|------|-------------|
| `requirements.txt` | Всегда — это рантайм |
| `requirements-gpu.txt` | Для ускорения на CUDA (пакеты cuBLAS + cuDNN) |
| `requirements-convert.txt` | Один раз, для конвертации моделей. Тянет torch и transformers |
...
### 4. Загрузите модель и сконвертируйте её

Движок работает с форматом CTranslate2, а не с safetensors от HuggingFace.
Скачайте модель Whisper с Hugging Face в папку `models/` и один раз сконвертируйте:
...
Скрипт просматривает `models/models--*/snapshots/*`, записывает результат в
`models/ct2/<имя>/` и работает полностью офлайн с уже имеющимися у вас весами.
Сначала посмотрите, что он нашёл: `python tools/convert_models.py --list`.

`whisper-medium` — рекомендуемый вариант, см. [Выбор модели](#-выбор-модели).

> При конвертации рядом с весами копируются `tokenizer.json` и
> `preprocessor_config.json`. Оба важны: без первого движок молча полезет в сеть,
> без второго модель со 128 мел-каналами (например `large-v3`) будет декодироваться
> как 80-канальная и тихо выдавать мусор. Без них конвертер откажется работать.

После конвертации исходные папки `models--*` нужны только для повторной
конвертации — их можно удалить и освободить место.

### 5. Запуск

```bash
Start_Whisper.bat        # двойной клик или запуск из терминала
# или
python start_gui.py
```
...
| Профиль | Режим | Для чего |
|---------|-------|----------|
| **Быстрый** | Пакетный, луч 1 | Черновики и быстрый проход по большому архиву |
| **Точный** *(по умолчанию)* | Пакетный, луч 5 | Повседневная работа. Настолько быстр, что реального компромисса с «Быстрым» нет |
| **Шумное аудио** | Последовательный, луч 5 | Плохие записи и всё, где нужны субтитры с нормальными таймингами |
